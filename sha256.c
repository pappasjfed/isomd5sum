...
static void SHA256_Transform(struct SHA256Context *ctx, const uint8 data[64])
{
...
void SHA256_Update(struct SHA256Context *ctx, const uint8 *data, size_t len)
{
    uint32 i;
...
void SHA256_Final(uint8 digest[32], struct SHA256Context *ctx)
{
    uint32 i;
    ...
    ctx->count[1] += ctx->count[0] * 8;
    ctx->buffer[63] = ctx->count[1];
    ctx->buffer[62] = ctx->count[1] >> 8;
    ctx->buffer[61] = ctx->count[1] >> 16;
    ctx->buffer[60] = ctx->count[1] >> 24;
    ctx->buffer[59] = ctx->count[0];
    ctx->buffer[58] = ctx->count[0] >> 8;
    ctx->buffer[57] = ctx->count[0] >> 16;
    ctx->buffer[56] = ctx->count[0] >> 24;
    SHA256_Transform(ctx, ctx->buffer);
